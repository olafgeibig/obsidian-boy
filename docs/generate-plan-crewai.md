<instructions>
Create a detailled project plan for the ObsidianBoy application as outlined in the existing draft project plan. The project will be implemented by an AI coding assistant using crewAI based on the project plan you generate. 
- Create a detailed concept including the agent responsibilities and description of the agent personas, their tasks and tools. Generate comprehensive agent and task definitions like in the crewAI examples, this is crucial for good agent performance.
- Define the crewAI solution concept:
  - Use pydantic models for concrete output formats of agents, e.g. the DailyNoteEntry class. Define these classes.
  - Make use of the crewAI pipeline feature that allows combining multiple crews. Maybe it makes sense to have sequential crews for sequential tasks like transforming daily notes into enriched entries and hierarchical crews for interactively working on refinements or a human in the loop.
  - Utilize the crewAI built-in tools
  - Utilize the crewAI memory where it is useful
  - Utilize suitable ObsidianBoy classes for crewAI as tools like the ObsidianInterface
- Update the PlantUML architecture diagram. 
- Break down the project into tasks that the coding assistent can work on task by task. Generate subtasks for complex tasks with concrete hints how to implement this with crewAI.
...
1. User selects daily notes. The terminal based interface allows the user to display a sorted list and select daily notes to be processed.
2. Daily note entry extraction. An LLM extracts entries from daily notes into a JSON format.
3. Entry enrichment. If important fields are missing, an LLM tries to fill the gaps like generating a title, finding the main website, summary of the entry.
4. Entry classification. An LLM classifies the entry and matches it to available note templates for different types of notes like tech-tool, business-idea, knowledge. The templates in markdown format are available in the file system and contain descriptions for the content of each section as a hint for the note writer LLM. 
5. Writing the notes. The note writer LLM performs research on each entry using web search, scraping and content analysis to gather the needed information and then writes the note based on the template using the researched knowledge. With feedbak loop.
6. Tagging the notes. An LLM tags new notes based on existing tags in Obsidian. With feedbak loop.
7. Distilled note update. An LLM finds and updates relevant distilled notes. With feedbak loop.
8. Human review an approval. New notes and updated distilled notes sre stored in a temp directory and then presented to the human for review. For the distilled notes a diff with the changes is shown. The human can approve, reject or keep new notes in the temp dir for manual editing.

## Solution Proposals

The solution is probably a mixture of fixed steps from the workflow while others may be better solved with an agentic approach. Probably LLM driven planning steps are beneficial. To be cost efficient the solution should use appropriate intelligent AI models for each step, ranging from simple and cheap models for easy tasks like summarization to more expensive capable models like planning. Probably memory should be added to the agents. 

## Architecture
This a a rough architecture draft, mostly covering the interface and specialized tools for the agents. The knowledge base is an abstract class that allows a simple file based knowledge base approach where the researched knowledge is stored in files that subsequently could be added to the context of LLM requests. Another implementation could use RAG. Some classes and methods are placeholders because the agentic flow was unclear at the time. The tools are partly implemented.
...
4. **WebScraper**: Responsible for scraping content from web resources.

5. **KnowledgeBase**: Abstract base class for knowledge storage and retrieval.

6. **FileSystemKnowledgeBase**: Concrete implementation of KnowledgeBase that stores data in the file system.

7. **MemoryManager**: Manages the storage and retrieval of content, and prepares context for LLM processing. It has an instance of KnowledgeBase.

8. **NoteTagger**, **DistilledNoteUpdater**: Handle various aspects of note creation and management.

9.  **HumanReviewInterface**: Handles the human review process, including presenting diffs for approval.

10. **DiffGenerator**: Generates diffs between original and updated notes.

11. **TerminalInterface**: Manages all terminal-based user interactions.
